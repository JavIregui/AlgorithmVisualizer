...
class SortingWindow:
    ...
    def show_algorithm_info(self):
        if not self.description_open:
            ...
            info_window.geometry(f"{300}x{200}+{self.win_x + self.win_width // 2 - 150}+{self.win_y + self.win_height // 2 - 100}")
            info_window.resizable(False, False)
            info_window.configure(bg="#303030")
            
            explanation = "Algorithm Explained"
            
            info_label = tk.Label(info_window, text=explanation, font=("Helvetica", 10), bg="#303030", fg="#FFFFFF", padx=10, pady=10)
            info_label.pack()
